...
func (te *TarExtractor) overlayFSWhiteout(dir string, file string) error {
	...
	if isOpaque {
		...
		return errors.Wrapf(err, "couldn't set whiteout attr for %s", dir)
	}

	// otherwise, white out the file itself.
	p := filepath.Join(dir, strings.TrimPrefix(file, whPrefix))
	if err := os.RemoveAll(p); err != nil && !os.IsNotExist(err) {
		return errors.Wrapf(err, "couldn't create whiteout for %s", p)
	}

	err := te.fsEval.Mknod(p, syscall.S_IFCHR|0666, unix.Mkdev(0, 0))
	return errors.Wrapf(err, "couldn't create whiteout for %s", p)
}
